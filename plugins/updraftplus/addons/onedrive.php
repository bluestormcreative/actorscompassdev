		...
		try {
			...
			if (false !== strpos($e->getMessage(), 'Optimistic concurrency failure during fragmented upload') && 0 == $this->onedrive_uploaded_size) {
				...
				try {
					...
					$commit = $service->apiPut('https://api.onedrive.com/v1.0/drive/root:/'. urlencode($this->onedrive_folder), $fp, null, strlen($commit_body), $commit_headers, true);
