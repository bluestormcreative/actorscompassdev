...
class GAPGoogle_Task_Runner
{
  ...
  /**
   * @var GAPGoogle_Client $client The current API client.
   */
  private $client;
   ...
   *
   * @param GAPGoogle_Client $client The current API client
   * @param string $name The name of the current task (used for logging)
   * @param callable $action The task to run and possibly retry
   * @param array $arguments The task arguments
   * @throws GAPGoogle_Task_Exception when misconfigured
   */
  public function __construct(
      GAPGoogle_Client $client,
      $name,
      $action,
      array $arguments = array()
  ) {
    $config = (array) $client->getClassConfig('GAPGoogle_Task_Runner');

    if (isset($config['initial_delay'])) {
      if ($config['initial_delay'] < 0) {
        throw new GAPGoogle_Task_Exception(
   ...
   *
   * @return mixed
   * @throws GAPGoogle_Task_Retryable on failure when no retries are available.
   */
  public function run()
  {
    while ($this->attempt()) {
      ...
      } catch (GAPGoogle_Task_Retryable $exception) {
