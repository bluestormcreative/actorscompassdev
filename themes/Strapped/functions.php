...
function strapped_scripts() {
	wp_enqueue_style( 'Strapped-styles', get_template_directory_uri() . '/css/bootstrap.min.css', array(), '3.3.5', 'all' );
	...
	wp_enqueue_script( 'Strapped-scripts', get_template_directory_uri() . '/js/scripts.js', array('jquery'), '3.3.5', false );

	wp_enqueue_script( 'Strapped-navigation', get_template_directory_uri() . '/js/bootstrap.min.js', array('jquery'), '3.3.5', true );

	wp_enqueue_script( 'Strapped-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );
...
function modified_trim_excerpt($text) {
	...
	if ( '' == $text ) {
			...
			if (count($words) > $excerpt_length) {
					array_pop($words);
					array_push($words, '[...] <p class="clear"><a class="read-more button" href="'. get_permalink($post->ID) . '">More</a></p>');
					$text = implode(' ', $words);
			}
			else {
				array_push($words, '<p class="clear"><a class="read-more button" href="'. get_permalink($post->ID) . '">More</a></p>');
